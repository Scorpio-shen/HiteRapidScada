...
namespace Scada.Comm.Devices
{
    ...
    public abstract partial class KPLogic
    {
        ...
        /// 如果为 true，则必须终止轮询会话或发送 KP 命令。
        /// 在轮询会话期间将其设置为 false 将暂停链接的末尾
        ...
        protected void AddEvent(KPEvent kpEvent)
        {
            lock (eventList)
            {
                // добавление события в список не переданных событий 将事件添加到未通过事件列表中
                eventList.Add(kpEvent);

                // добавление события в список последних событий 将事件添加到最近事件列表
