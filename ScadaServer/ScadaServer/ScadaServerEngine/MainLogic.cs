...
namespace Scada.Server.Engine
{
    ...
    sealed partial class MainLogic : IServerData
    {
        ...
        private SortedList<int, InCnl> inCnls;     // активные входные каналы 有源输入通道
        ...
        private void Execute()
        {
            try
            {
                ...
                while (!terminated)
                {
                    ...
                    //在将时间向后移动或在循环的第一次通过时计算切片的记录时间并计算预计算通道的值
                    if (prevDT > nowDT)
                    {
                    ...
                    //清除过时的缓存数据
                    if (nowDT - clearCacheDT > CacheClearSpan || nowDT < clearCacheDT /*время переведено назад 时光倒流*/)
                    {
                        clearCacheDT = nowDT;
        ...
        private void WriteSrez(SnapshotType srezType, DateTime srezDT)
        {
            ...
            else
            {
                ...
                // получение кэша таблицы срезов 获取切片表缓存
        ...
        /// <summary>
        /// Записать срез в таблицу текущего среза 将切片写入当前切片表
        /// </summary>
        ...
        /// <summary>
        /// Записать срез в таблицу архивных (минутных или часовых) срезов 将切片写入存档（分钟或小时）切片表
        /// </summary>
            ...
            SrezTableLight.Srez receivedSrez, DateTime srezDT, ref SrezTableLight.Srez arcSrez)
        {
            ...
            try
            {
                // получение существующего или создание нового архивного среза 获取现有的或创建新的存档切片
                fileName = srezAdapter.FileName;
                SrezTableCache.FillSrezTable(srezTable, srezAdapter);
                ...
                lock (calculator)
                {
                    try
                    {
                        ...
                        for (int i = 0; i < cntCnt; i++)
                        {
                            ...
                            if (inCnls.TryGetValue(cnlNum, out InCnl inCnl) && cnlInd >= 0) // входной канал существует 输入通道存在
                            {
        ...
        /// <summary>
        /// Вызвать событие OnCommandReceived для модулей 引发模块的 OnCommandReceived 事件
        /// </summary>
        ...
        /// <summary>
        /// Обработать новые архивные данные   、、处理新的存档数据
        /// </summary>
            ...
            try
            {
                if (serverIsReady)
                {
                    ...
                    if (cnlCnt > 0)
                    {
                        ...
                        // выполнение действий модулей 执行模块操作
        ...
        /// <summary>
        /// Установить данные входного канала обрабатываемого среза 设置已处理切片的输入通道数据
        /// </summary>
        /// <remarks>Метод используется для вычисления по формулам базы конфигурации</remarks>
        ...
        public void ProcCommand(CtrlCnl ctrlCnl, Command cmd, int userID, out bool passToClients)
        {
            ...
            try
            {
                if (serverIsReady && ctrlCnl != null && cmd != null)
                {
                    ...
                    // вычисление значения или данных команды по формуле канала управления 通过控制通道公式计算指令值或数据
                    if (ctrlCnl.CalcCmdVal != null)
                    {
                    ...
                    // выполнение действий модулей после приёма команды 收到命令后执行模块动作
                    RaiseOnCommandReceived(ctrlCnlNum, cmd, userID, ref passToClients);

                    // запись события и выполнение действий модулей 记录事件并执行模块动作
