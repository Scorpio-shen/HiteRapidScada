...
namespace Scada.Server.Engine
{
    ...
    sealed class Comm : IServerCommands
    {
        ...
        /// <summary>
        /// Получить описание команды 获取命令描述
        /// </summary>
        ...
        private void ProcCommCommand(ClientInfo client, byte cmd, int cmdLen)
        {
            ...
            switch (cmd)
            {
                case 0x01: // проверка имени и пароля 名称和密码验证
                    ...
                    break;
                case 0x02: // запрос состояния сервера (ping) 服务器状态查询
                    respDataLen = 1;
                    outBuf[3] = mainLogic.ServerIsReady ? (byte)1 : (byte)0;
                    break;
                case 0x03: // запись текущего среза 记录当前切片
                    ...
                    respDataLen = 1;
                    break;
                case 0x04: // запись архивного среза 存档切片录制
                    if (client.UserRoleID == BaseValues.Roles.App)
                    {
                ...
                case 0x06: // команда ТУ
                    bool cmdProcOk = false; // команда обработана успешно 命令处理成功
                ...
                case 0x07: // запрос команды ТУ
                    ...
                    break;
                case 0x08: // открытие и чтение из файла 打开和读取文件
                    int readCnt = 0;
                    bool readOk = false;
                    ...
                    break;
                case 0x09: // перемещение позиции чтения из файла  、、从文件中移动读取位置
                    long pos = 0;
                    bool seekOk = false;
                    ...
                    break;
                case 0x0A: // чтение из файла  、、从文件读取
                    readCnt = 0;
                ...
                case 0x0B: // закрытие файла
                    ...
                    respDataLen = 1;
                    break;
                case 0x0C: // запрос времени изменения файлов 文件修改时间查询
                    int fileCnt = inBuf[3];
                    outBuf[3] = inBuf[3];
                    ...
                    break;
                case 0x0D: // запрос данных из таблицы среза 从切片器表中查询数据
                    ...
                    break;
                case 0x0E: // квитирование события 事件握手
                    if (client.Authenticated)
                    {
