        ...
        protected bool ReceiveFileToStream(Dirs dir, string fileName, Stream inStream)
        {
            ...
            try
            {
                ...
                while (!stop)
                {
                    if (open)
                    {
                        // отправка команды открытия файла и чтения данных 发送命令打开文件并读取数据
                    ...
                    {
                        // отправка команды чтения данных из файла 发送命令从文件中读取数据
                        buf[0] = 0x05;
                        buf[1] = 0x00;
                        buf[2] = 0x0A;
                        buf[3] = DataSizeL;
                    ...
                    // приём результата открытия файла и считанных данных 接收打开文件的结果和读取的数据
                    byte cmdNum = buf[2];
                    int headerLen = open ? 6 : 5;
                    int bytesRead = netStream.Read(buf, 0, headerLen);
                    int dataSizeRead = 0; // размер считанных из файла данных  从文件中读取的数据大小                  
        ...
        protected bool SendCommand(int userID, int ctrlCnl, double cmdVal, byte[] cmdData, int kpNum, out bool result)
        {
            ...
            try
            {
                if (RestoreConnection())
                {
                    ...
                    if (!double.IsNaN(cmdVal)) // стандартная команда 标准命令
                    {
                        buf[5] = 0x00;
                        buf[8] = 0x08;
                        buf[9] = 0x00;
                    ...
                    else if (cmdData != null) // бинарная команда 二进制命令
                    {
                        buf[5] = 0x01;
        ...
        public bool SendEvent(EventTableLight.Event aEvent, out bool result)
        {
            ...
            try
            {
                if (RestoreConnection())
                {
                    ...
                    // отправка команды записи события 发送事件记录命令
