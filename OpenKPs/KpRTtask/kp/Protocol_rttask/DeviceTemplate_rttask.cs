...
namespace Scada.Comm.Devices.rttask.Protocol
{
    //mw
    public static class RTTASK_BaseValues
    {
        //public static class PLC_shm_column_text
        //{
        //    public const string plc_addr_type = "plc_addr_type"; //PLC地址类型
        //    public const string plc_addr = "plc_addr"; //PLC数据地址
        //    public const string rttask_type = "rttask_type"; //RTTASK数据类型
        //}

        public static class PLC_addr_types_text
        {
            public const string addrM = "M";
            public const string addrSPM = "SPM";
            public const string addrD = "D";
            public const string addrSPD = "SPD";
            public const string addrX = "X";
            public const string addrY = "Y";
            public const string addrS = "S";
            public const string addrT = "T";
            public const string addrC = "C";
        }

        public static class PLC_addr_types
        {
            public const int addrM = 0;
            public const int addrSPM = 1;
            public const int addrD = 2;
            public const int addrSPD = 3;
            public const int addrX = 4;
            public const int addrY = 5;
            public const int addrS = 6;
            public const int addrT = 7;
            public const int addrC = 8;
        }

        public static class RTTASK_data_types_text
        {
            public const string BOOL = "BOOL";
            public const string U8 = "UCHAR";
            public const string U16 = "USHORT";
            public const string U32 = "UINT";
            public const string U64 = "ULONGLONG";
            public const string S8 = "CHAR";
            public const string S16 = "SHORT";
            public const string S32 = "INT";
            public const string S64 = "LONGLONG";
            public const string FLOAT = "FLOAT";
            public const string DOUBLE = "DOUBLE";
        }

        public static class RTTASK_data_types
        {
            public const int BOOL = 0;
            public const int U8 = 1;
            public const int U16 = 2;
            public const int U32 = 3;
            public const int U64 = 4;
            public const int S8 = 5;
            public const int S16 = 6;
            public const int S32 = 7;
            public const int S64 = 8;
    ...
    public class DeviceTemplate
    {
        ...
        //mw
        //Elems and Cmds
        public DataTable dtElems;
        public DataTable dtShm;
        ...
        /// </summary>
        public void SetCmdData(double cmdVal, DataRow dr)
        {
            bool reverse = true;

            switch ((string )dr["RTTASK数据类型"])
            {
                case "USHORT":
        ...
        //mw
        public DataRow FindShmCmd(int cmdNum)
        {
            if (dtElems != null || cmdNum < dtElems.Rows.Count)
            {
                return dtElems.Rows[cmdNum];
            }
        ...
        //mw
        public bool Load(string fileName, out string errMsg, ref DataTable dt)
        {
            SetToDefault();
            ...
            try
            {
                XmlDocument xmlDoc = new XmlDocument();
                xmlDoc.Load(fileName);
                LoadFromXml(xmlDoc.DocumentElement, ref dt);
                errMsg = "";
                return true;
            }
            catch (Exception ex)
        ...
        //mw
        public void LoadFromXml(XmlNode rootNode, ref DataTable dt)
        {
            XmlNode ElemsNode = rootNode.SelectSingleNode("Elems");
            //DataTable dt = new DataTable();
            dtElems.Clear();

            dtElems.Columns.Add("变量名", typeof(string)); //变量名
            dtElems.Columns.Add("数据类型", typeof(string)); //PLC数据类型
            dtElems.Columns.Add("变量地址", typeof(string));
            dtElems.Columns.Add("变量描述", typeof(string));
            dtElems.Columns.Add("RTTASK数据类型", typeof(string));
            foreach (XmlElement elemElem in ElemsNode.SelectNodes("Elem"))
            {
                DataRow dr = dtElems.NewRow();
                dr["变量名"] = elemElem.GetAttribute("name");
                dr["数据类型"] = elemElem.GetAttribute("type");
                dr["变量地址"] = elemElem.GetAttribute("addr");
                dr["变量描述"] = elemElem.GetAttribute("comment");
                dr["RTTASK数据类型"] = elemElem.GetAttribute("rttask_type");
                dtElems.Rows.Add(dr);
            }
        ...
        //mw
        public bool Save(string fileName, out string errMsg, DataTable dt)
        {
            try
            {
                XmlDocument xmlDoc = new XmlDocument();
                ...
                if (dt == null)
                {
                    errMsg = "datatable is Null.";
                    return false;
                }
                XmlElement elemElems = rootElem.AppendElem("Elems");
                //Elems.SetAttribute("", );
                foreach (DataRow dr in dt.Rows)
                {
                    XmlElement elemElem = elemElems.AppendElem("Elem");
                    if (dt.Columns.Contains("变量名"))
                    {
                        elemElem.SetAttribute("name", (string)dr["变量名"]);
                    }

                    if (dt.Columns.Contains("数据类型"))
                    {
                        elemElem.SetAttribute("type", (string)dr["数据类型"]);
                    }

                    if (dt.Columns.Contains("变量地址"))
                    {
                        elemElem.SetAttribute("addr", (string)dr["变量地址"]);
                    }

                    if (dt.Columns.Contains("变量描述"))
                    {
                        elemElem.SetAttribute("comment", (string)dr["变量描述"]);
                    }

                    if (dt.Columns.Contains("RTTASK数据类型"))
                    {
                        elemElem.SetAttribute("rttask_type", (string)dr["RTTASK数据类型"]);
                    }
                }
