...
using Scada.Comm.Devices.rttask.UI;
...
namespace Scada.Comm.Devices
{
    ...
    /// </summary>
    public class KpRTtaskView : KPView
    {
        /// <summary>
        /// The driver version.
        /// </summary>
        ...
        /// </summary>
        public KpRTtaskView(int number)
            : base(number)
        {
            CanShowProps = true;
        }
        ...
        public override KPCnlPrototypes DefaultCnls
        {
            get
            {
                ...
                if (!deviceTemplate.Load(filePath, out string errMsg, ref dt))
        ...
        protected virtual KPCnlPrototypes CreateCnlPrototypes(DeviceTemplate deviceTemplate)
        {
            ...
            {
                InCnlPrototype inCnl;
                if ((string )dr["RTTASK数据类型"] == "BOOL")
                {
                    inCnl = new InCnlPrototype((string)dr["变量地址"], BaseValues.CnlTypes.TS);
                    inCnl.ShowNumber = false;
                    inCnl.UnitName = BaseValues.UnitNames.OffOn;
                    inCnl.EvEnabled = true;
                    inCnl.EvOnChange = true;
                }
                else //"WORD" "LONG" "FLOAT",后面调试需要注意判断
                {
                    inCnl = new InCnlPrototype((string)dr["变量地址"], BaseValues.CnlTypes.TI);
                }
                inCnl.Signal = signal++;
                inCnls.Add(inCnl);
            }
            ...
            foreach (DataRow dr in deviceTemplate.dtElems.Rows)
            {
                CtrlCnlPrototype ctrlCnl;
                if ((string)dr["RTTASK数据类型"] == "BOOL")
                {
                    ctrlCnl = new CtrlCnlPrototype((string)dr["变量地址"], BaseValues.CmdTypes.Binary);
                    ctrlCnl.CmdValName = BaseValues.CmdValNames.OffOn;
                }
                else //"WORD" "LONG" "FLOAT",后面调试需要注意判断
                {
                    ctrlCnl = new CtrlCnlPrototype((string)dr["变量地址"], BaseValues.CmdTypes.Standard);
                }
                ctrlCnl.CmdNum = CmdNum++;
        ...
        {
            if (!Localization.LoadDictionaries(AppDirs.LangDir, "Kprttask", out string errMsg))
        ...
        public override void ShowProps()
        {
            ...
            else
            {
                ...
                FrmDevTemplate_rttask.ShowDialog(AppDirs, GetUiCustomization());
            }
        }
    }
}
