...
namespace HslCommunication.Core.Security
{
	...
	public class AesCryptography : ICryptography
	{
		...
		/// <inheritdoc cref="ICryptography.Encrypt(string)"/>
		public string Encrypt( string data )
		{
			byte[] buffer = string.IsNullOrEmpty( data ) ? new byte[0] : Encoding.UTF8.GetBytes( data );
			return Convert.ToBase64String( Encrypt( buffer ) );
		}

		/// <inheritdoc cref="ICryptography.Decrypt(string)"/>
