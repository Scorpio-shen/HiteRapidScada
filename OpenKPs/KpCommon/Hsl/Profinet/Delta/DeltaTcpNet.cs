...
namespace HslCommunication.Profinet.Delta
{
	...
	/// </example>
	public class DeltaTcpNet : ModbusTcpNet, IDelta
	{
		#region Constructor

		/// <inheritdoc cref="DeltaSerial()"/>
		public DeltaTcpNet( ) : base( ) { ByteTransform.DataFormat = DataFormat.CDAB; }

		/// <inheritdoc cref="ModbusTcpNet(string, int, byte)"/>
		public DeltaTcpNet( string ipAddress, int port = 502, byte station = 0x01 ) : base( ipAddress, port, station ) { ByteTransform.DataFormat = DataFormat.CDAB; }
		...
		/// <inheritdoc/>
		public override string ToString( ) => $"DeltaTcpNet[{IpAddress}:{Port}]";
