...
namespace HslCommunication.Profinet.AllenBradley
{
	...
	/// 如果需要返回10个长度的short数组，请调用 ReadInt16("A[0], 10"); 地址必须写 "A[0]"，不能写 "A" , 如需要读取结构体，参考 <see cref="Omron.OmronConnectedCipNet.ReadStruct{T}(string)"/>
	/// </remarks>
	/// <example>
	...
	///     <term>0xC6</term>
	///     <term>byte</term>
	///     <term>如需要，使用<see cref="Omron.OmronConnectedCipNet.WriteTag(string, ushort, byte[], int)"/>实现</term>
	///   </item>
	///   <item>
	///     <term>BYTE</term>
	///     <term>8位字符数据</term>
	...
	public class AllenBradleyConnectedCipNet : Omron.OmronConnectedCipNet, IAbReadWriteCip
    {
		...
		/// 实例化一个默认的对象
		/// </summary>
		public AllenBradleyConnectedCipNet( ) : base( )
		{
		}
		...
		public AllenBradleyConnectedCipNet( string ipAddress, int port = 44818 ) : base( ipAddress, port )
		{
		}
		...
		/// <inheritdoc/>
		protected override int GetMaxTransferBytes( ) => 1980;
