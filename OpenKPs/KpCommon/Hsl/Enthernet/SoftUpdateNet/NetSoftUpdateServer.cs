...
namespace HslCommunication.Enthernet
{
	...
	public sealed class NetSoftUpdateServer : NetworkServerBase
	{
		...
		public long GetDealSizeAndReset( ) => Interlocked.Exchange( ref downloadSize, 0 );
...
#endif
		{
			...
			else
			{
				...
				AppSession session = new AppSession( socket );
				session.Tag = HslTimeOut.HandleTimeOutCheck( socket, 1000 * 60 * 30 );   // 超过30分钟没有更新好，则直接超时
				lock (lockSessions) sessions.Add( session );
				try
				{
					// 安装系统和更新系统
					...
					for (int i = 0; i < files.Length; i++)
					{
						...
						fileName = RemoveHeadPathChar( fileName );
				...
				{
					if (session.Tag is HslTimeOut hslTimeOut) hslTimeOut.IsSuccessful = true;
					RemoveAndCloseSession( session );
