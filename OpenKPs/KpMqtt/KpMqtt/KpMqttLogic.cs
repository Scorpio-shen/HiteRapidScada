...
namespace Scada.Comm.Devices
{
    public class KpMqttLogic : KPLogic
    {
        ...
        private void InitSubscribePacket()
        {
            ...
            {
                //WriteToLog("MQTT_CMD_Sub:" + subCmd.TopicName); // 测试使用 RMY 2022 02 16
        ...
        private void OnPublishReceived(PublishPacket packet)
        {
            ...
            if (subCmds.TryGetValue(packet.Topic, out MqttSubCmd subCmd))
            {
                switch (subCmd.CmdType)
                {
                    ...
                    case CmdType.BinHex:
                       // WriteToLog(packetMsg);
                        if (ScadaUtils.HexToBytes(packetMsg.Trim(), out byte[] cmdData))
                        {
                            //WriteToLog(cmdData.ToString());

                           // int nSize = cmdData.Length;

                           // for (int i = 0; i < nSize; i++)
                           // {
                           //     WriteToLog(cmdData[i].ToString());
                           // }
                           // cmdData[1] = 0;
        ...
        public override void SendCmd(Command cmd)
        {
            ...
            WriteToLog("发送命令 " + cmd.ToString()); // rmy 2022 02 16
            //Logger.Debug("Mqtt 发送命令 " + cmd.ToString());
            ...
            else
            {
                foreach (MqttPubCmd pubCmd in deviceConfig.PubCmds)
                {
                    if (pubCmd.NumCmd == cmd.CmdNum)
                    {
                        ...
                        {
                            //WriteToLog("发送命令 Binary" + cmd.CmdData.Length); // rmy 2022 02 16

                            //string str = BitConverter.ToString(cmd.CmdData, 0, cmd.CmdData.Length);

                            //WriteToLog("发送命令 Binary" + str);
