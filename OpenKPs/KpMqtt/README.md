KpMqtt
=============================
The KpMqtt.dll driver is an application library for the Scada Communicator of the RS project. Using this driver, can be receive and transmit messages using the MQTT protocol.
...
Typically, a description of most hierarchical objects can be represented as a tree-like structure. For example, a description of access to RS channels could be presented in the form of the following template for a tree branch:
树状结构 
访问RS通道的模板 可以用对象  设备号  通道号
/rapidobject/rapidkp/rapidcnl , where

-	**Rapidobject** indicates a unique object number from the RS object directory(对象目录唯一的对象号);
-	**Rapidkp** indicates a unique device number from the devices directory(设备目录的唯一设备号);
-	**Rapidcnl** indicates the channel number from the directory of input channels(输入通道目录的通道号);
...
The MQTT protocol description [page](http://mqtt.org/) contains information on various implementations of client and server applications. Most often, [Mosquitto server](http://mosquitto.org/)  is used as a MQTT broker for small IoT projects. 
The project site has a lot of documentation that will allow you to deploy a server for various operating systems, including the Windows operating system. [The Mosquitto project](http://mosquitto.org/)   is actively supported and complies with the latest versions of the MQTT protocol specification. 
The software packages of this broker are also available for routers based on the OpenWRT OS. There are also open-source projects for installing enterprise-wide brokers that can withstand high loads and provide fault tolerance mechanisms. If it is not possible to install your own MQTT broker, then can be use the services of cloud services deployed by third-party companies. 
...
KpMqtt driver setup can be divided into 2 stages: （云服务）
- Creating a configuration file for the **KpMqtt** driver（配置KpMqtt驱动配置）
- Creating a communication line, adding device and linking to it a configuration file（添加通讯线路，增加设备与驱动配置文件关联）
When creating a configuration file for the KpMqtt driver, the following block sections and their attributes are used:（配置驱动文件需要配置的属性信息）
...
-	**RapSrvCnf** section - this section contains attributes designed to connect to the RS server.（边缘控制侧的服务端口 IP）
	-	**ServerHost** attribute - this attribute must contain the IP address of the RS server.
	-	**ServerPort**  attribute - this attribute must contain the port number on which the RS server accepts connections from client applications..
	-	**ServerUser** and **ServerPwd** attributes - these attributes contain the username and password for accessing the RS server.

-	**MqttSubTopics** section - this section is the parent of the Topic section and is used to configure topic subscriptions.（主题订阅）

-	**MqttPubTopics** section - this section is the parent of the Topic section and is used to configure topic publishing.（主题发布）

-	**MqttPubCmds** section - this section is the parent of the Topic section and is used to configure the publication of RS commands.（ 配置发布 边缘控制侧指令）
...
-	**Topic** section - this section is a child of the MqttPubTopics, MqttSubTopics, MqttPubCmds, MqttSubCmds, MqttSubJSs sections and is intended for topic configuration.
	...
	-	**NumCnl** attribute - this attribute defines the channel number from the RS configuration table（输入通道编号）.
...
"BinTxt" - a binary control command is formed. The input value format should be in the form of a normal UTF-8 string. 
"Req" - the command of an extraordinary poll of the CP is formed. The input value is ignored. The value of the KpNum attribute is used.
...
There are no restrictions on the number of created devices within the same communication line for this driver. But at the same time, other conditions should be taken into account, which can have a significant impact on the entire mechanism of the communication line as a whole. 
If we take into account external conditions, then you should pay attention to the configuration parameter of the **MQTT** broker timeout. The total polling cycle of all devices on one communication line should not exceed the broker's timeout.
When creating more than one **KpMqtt** device on the same communication line, you must specify different **ClientIDs**.
